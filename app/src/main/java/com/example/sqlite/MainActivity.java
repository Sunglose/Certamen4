...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Error 1 (faltan adaptadores para los spinners.)
        ArrayAdapter<String> adaptadorCat = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, CategoriaS);
        Categoria.setAdapter(adaptadorCat);

        ArrayAdapter<String> adaptadorUbi = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, UbicacionS);
        Ubicacion.setAdapter(adaptadorUbi);
    }
    ...
    public void RegistrarProducto(View view){
        ...
        if (!fila.moveToFirst()) {
            ...
            Toast.makeText(this, "El ID del producto ya está registrado.", Toast.LENGTH_SHORT).show();
        }
    }
    public void BuscarProducto(View view){
        //Error 2 (El nombre de la base de datos no es correcto. Mal= "Control_Inventario | Bueno= "ControlInventario)
        AdminSQLiteOpenHelper admin = new AdminSQLiteOpenHelper(this, "ControlInventario", null, 1);
        SQLiteDatabase BaseDatos = admin.getWritableDatabase();
    ...
    public void EliminarProducto(View view){
        ...
        if (!IDProducto.isEmpty()){
            // Error 4 (El whereClause no tiene el nombre correcto. Mal= IDProducto | Bueno= ID)
            int Eliminar = BaseDatos.delete("Inventario_Bodega", "ID=" +IDProducto, null);
            if (Eliminar == 1){
                Toast.makeText(this, "Se han eliminado los datos correctamente.", Toast.LENGTH_LONG).show();
    ...
    public void ModificarProducto(View view){
        ...
        // Error 5 (CategoriaS estaba como Integer al igual que precio)
        String IDproducto = ID.getText().toString();
        String NombreProducto = Nombre.getText().toString();
        String CategoriaS = Categoria.getSelectedItem().toString();
        String cantidad = Cantidad.getText().toString();
        ...
        if (!IDproducto.isEmpty() && !NombreProducto.isEmpty() && !CategoriaS.equals("Seleccione:") && !cantidad.equals("") && !precio.equals("") && !ubicacion.equals("Seleccione:")){
            ...
            //Error 3 (Falta toast para notificar si los datos se actualizaron correctamente.)
            if (Update == 1){
                Toast.makeText(this, "Se han actualizado los datos.", Toast.LENGTH_LONG).show();
            }else{
                Toast.makeText(this, "No se encontró el producto que busca.", Toast.LENGTH_SHORT).show();
            }
        } else {
